...
skip
  ...
  .describe("PriceExercise Integration Tests", async function () {
    ...
    beforeEach(async () => {
      ...
      priceExercise = await ethers.getContractAt(
        "PriceExercise",
    ...
    it("price feed return a positive value", async () => {
      ...
      console.log(
        "Price Feed Value: ",
        new web3.utils.BN(result._hex).toString()
      );
      expect(
        new web3.utils.BN(result._hex).toString()
      ).to.be.a.bignumber.that.is.greaterThan(new web3.utils.BN(0));
    });
    ...
    it("Should successfully make an external API request and get a result", async () => {
      ...
      console.log(
        "Price obtained from API Call: ",
        new web3.utils.BN(result._hex).toString()
      );
      expect(
        new web3.utils.BN(result._hex)
      ).to.be.a.bignumber.that.is.greaterThan(new web3.utils.BN(0));
    });
  });
